...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productMovement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-movements.update", [$productMovement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $productMovement->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('remark') ? 'is-invalid' : '' }}" name="remark" id="remark">{{ old('remark', $productMovement->remark) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('in_location') ? 'is-invalid' : '' }}" name="in_location_id" id="in_location_id" required>
                    @foreach($in_locations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('in_location_id') ? old('in_location_id') : $productMovement->in_location->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('record_in_at') ? 'is-invalid' : '' }}" type="text" name="record_in_at" id="record_in_at" value="{{ old('record_in_at', $productMovement->record_in_at) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('record_in_by') ? 'is-invalid' : '' }}" name="record_in_by_id" id="record_in_by_id" required>
                    @foreach($record_in_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('record_in_by_id') ? old('record_in_by_id') : $productMovement->record_in_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('out_location') ? 'is-invalid' : '' }}" name="out_location_id" id="out_location_id">
                    @foreach($out_locations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('out_location_id') ? old('out_location_id') : $productMovement->out_location->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('record_out_at') ? 'is-invalid' : '' }}" type="text" name="record_out_at" id="record_out_at" value="{{ old('record_out_at', $productMovement->record_out_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('record_out_by') ? 'is-invalid' : '' }}" name="record_out_by_id" id="record_out_by_id">
                    @foreach($record_out_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('record_out_by_id') ? old('record_out_by_id') : $productMovement->record_out_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('finish_at') ? 'is-invalid' : '' }}" type="text" name="finish_at" id="finish_at" value="{{ old('finish_at', $productMovement->finish_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('record_finish_by') ? 'is-invalid' : '' }}" name="record_finish_by_id" id="record_finish_by_id">
                    @foreach($record_finish_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('record_finish_by_id') ? old('record_finish_by_id') : $productMovement->record_finish_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
